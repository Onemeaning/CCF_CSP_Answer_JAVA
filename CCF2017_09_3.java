...
public class CCF2017_09_3 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < input.length; i++)
		{
			...
			{
				sBuffer.append(line.substring(0, index));
				sBuffer.append(line.substring(index+1,index+2));
			...
			if ((index = temp[1].indexOf("{"))!=-1)
			{
				flag = key;	
				jsons.put(key, "OBJECT");
				temp[1] = temp[1].substring(index+1);
			}
			
			if ((index = temp[1].indexOf("}"))!=-1)
			{				
				String value = temp[1].substring(1,temp[1].length()-2);
			    jsons.put(flag+"."+key, value);
				flag = "";
				continue;			
			}
			
			if (flag.length() > 0 && temp[1].trim().length()>1)
			{
...
//		for(Entry<String, String> kEntry : jsons.entrySet())
//		{
